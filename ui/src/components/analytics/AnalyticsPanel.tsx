...
export const FeatureTrendChart: React.FC<{ className?: string }> = ({ className }) => {
  const timeSeries = useStore(s => s.timeSeries);
  const data = useMemo(() => timeSeries.slice(-120), [timeSeries]);

  return (
    <Card title="Feature Trends — CFER · Turbulence · Shockwave · Entropy" className={className}>
...
export const ThreatScoreChart: React.FC<{ className?: string }> = ({ className }) => {
  const timeSeries = useStore(s => s.timeSeries);
  const data = useMemo(() => timeSeries.slice(-180), [timeSeries]);

  return (
    <Card title="Threat Score — Live Timeline" className={className} glow={
...
export const SeverityDistributionChart: React.FC<{ className?: string }> = ({ className }) => {
  ...
  return (
    <Card title="Severity Distribution — 30-Minute Buckets" className={className}>
...
export const CFERDetailChart: React.FC<{ className?: string }> = ({ className }) => {
  const timeSeries = useStore(s => s.timeSeries);
  const data = useMemo(() => timeSeries.slice(-120), [timeSeries]);

  return (
    <Card title="CFER (Capability Field Expansion Rate)" subtitle="Linear regression slope of ||F|| over time window" className={className}>
      <div className="flex-1 min-h-[80px]">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
            <defs>
...
export const EntropyTurbulenceChart: React.FC<{ className?: string }> = ({ className }) => {
  const timeSeries = useStore(s => s.timeSeries);
  const data = useMemo(() => timeSeries.slice(-120), [timeSeries]);

  return (
    <Card title="Entropy & Turbulence" subtitle="Spread indicator vs activity variance" className={className}>
      <div className="flex-1 min-h-[80px]">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" />
...
export const AnalyticsPanel: React.FC = () => (
  <div className="h-full flex flex-col gap-4">
    <ThreatScoreChart className="flex-1 min-h-0" />
    <FeatureTrendChart className="flex-1 min-h-0" />
    <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-2 gap-4">
      <CFERDetailChart className="h-full" />
      <EntropyTurbulenceChart className="h-full" />
    </div>
    <SeverityDistributionChart className="flex-1 min-h-0" />
  </div>
);
